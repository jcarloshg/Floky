import 'package:floky/dependencyInjection/response_activities/di.get_activities_by_key_type.dart';
...
import 'package:floky/domain/usecase/response_activities/infrastructure/aws/aws.get_activities_by_type.dart';
...
Future<void> responseActivities({required GetIt di}) async {
  ...
  final ResponseActivities domain = ResponseActivities(
    ...
    getActivitiesByTypeRepository: GetActivitiesByTypeAWS(),
    getCurrentStudentRepository: GetCurrentStudentAWS(),
  );

  await getRecentActivities(di: di, domain: domain, navigator: navigator);
  await getActivitiesByKeyType(di: di, domain: domain, navigator: navigator);
